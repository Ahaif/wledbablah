...
async function fundContractWithDAI(contractAddress: string, daiAmount: string) {
    ...
    try{
        await fundDAIHolderWithETH(daiHolderAddress, "1"); // Send 0.1 ETH to DAI holder for gas
...
async function main() {
    try{
        ...
        await fundContractWithDAI(contractAddress, "1000000"); // Amount of DAI you want to transfer
    }
    catch(error : any)
    {
        console.log("Error: ", error.message);
